//
//  SignInWithAppleCoordenator.swift
...
@available(iOS 13.0, *)
extension SigInWithAppleCoordinator: ASAuthorizationControllerDelegate {
      ...
      guard let appleIDToken = appleIDCredential.identityToken else {
        ...
        Auth.auth().currentUser?.link(with: credential, completion: { authresult, error in
            if let error = error, (error as NSError).code == AuthErrorCode.credentialAlreadyInUse.rawValue {
                print("User is already liked")
                if let updatedCredential = (error as NSError).userInfo[AuthErrorUserInfoUpdatedCredentialKey] as? OAuthCredential {
                    print("Signin using updated credentials")
